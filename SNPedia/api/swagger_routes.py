...
def create_swagger_blueprint() -> Blueprint:
    ...
    rsid_model = api.model(
        "RSID",
        {
            ...
            "Variations": fields.String(
                description="HTML-formatted variation information"
            ),
            "StabilizedOrientation": fields.String(
                description="Stabilized orientation from SNPedia"
            ),
            "IsInteresting": fields.String(
                description="Whether SNP is interesting (Yes/No)"
            ),
            "IsUncommon": fields.String(
                description="Whether genotype is uncommon (Yes/No)"
            ),
        },
    )
    ...
    pagination_model = api.model(
        "Pagination",
        {
            ...
            "total_pages": fields.Integer(
                required=True, description="Total number of pages"
            ),
            "has_next": fields.Boolean(
                required=True, description="Whether there is a next page"
            ),
    ...
    paginated_rsids_model = api.model(
        "PaginatedRSIDs",
        {
            "results": fields.List(
                fields.Nested(rsid_model), required=True, description="List of RSIDs"
            ),
    ...
    health_model = api.model(
        "Health",
        {
            ...
            "data_loaded": fields.Boolean(
                required=True, description="Whether data is loaded"
            ),
            "data_count": fields.Integer(
                required=True, description="Number of data items loaded"
            ),
            "config_valid": fields.Boolean(
                required=True, description="Whether configuration is valid"
            ),
            "config_warnings": fields.List(
                fields.String, description="Configuration warnings"
            ),
    ...
    class RSIDList(Resource):
        ...
        @rsids_ns.param(
            "page_size", "Items per page (optional, max 1000)", type="integer"
        )
    ...
    class CacheInvalidate(Resource):
        ...
        def post(self, filename: str) -> Any:
