    ...
    if os.path.exists(results_file):
        try:
            with open(results_file, 'r') as f:
                ...
                if existing_data:
                    existing_rsids = set(rsid.lower() for rsid in existing_data.keys())
                    logger.info(f"Found existing SNPedia data for {len(existing_rsids)} RSIDs")
...
def crawl_snpedia(use_async: bool = True):
    ...
    try:
        ...
        logger.info(f"Found {len(missing_rsids)} RSIDs to crawl from SNPedia (out of {len(valid_snps)} total)")
...
def process_results():
    ...
    try:
        ...
        if success:
            logger.info(f"Successfully processed and saved {len(enriched_snps)} enriched SNPs")
            return True
        else:
...
def main():
    ...
    import_parser.add_argument('-f', '--file', required=True, 
                              help='Path to genome data file')
    ...
    crawl_parser.add_argument('--sync', action='store_true', 
                             help='Use synchronous crawling (default: async)')
    ...
    pipeline_parser = subparsers.add_parser('pipeline', 
                                           help='Run full pipeline: import -> crawl -> process')
    pipeline_parser.add_argument('-f', '--file', required=True,
                                help='Path to genome data file')
    pipeline_parser.add_argument('--sync', action='store_true',
                                help='Use synchronous crawling (default: async)')
