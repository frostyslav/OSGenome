...
class Config:
    ...
    if not SECRET_KEY:
        ...
        logging.warning(
            "Using randomly generated SECRET_KEY. Set SECRET_KEY environment variable for production."
        )
    ...
    ALLOWED_EXTENSIONS = set(
        os.environ.get("ALLOWED_EXTENSIONS", "xlsx,xls").split(",")
    )

    # Data processing limits
    MAX_FILE_SIZE_IMPORT = get_env_int(
        "MAX_FILE_SIZE_IMPORT", 100 * 1024 * 1024
    )  # 100MB
    ...
    REQUEST_DELAY = get_env_float(
        "REQUEST_DELAY", 1.5
    )  # seconds - increased for stability
    ...
    def validate(cls: type["Config"]) -> Dict[str, Any]:
        ...
        if cls.SECRET_KEY is None:
            ...
            warnings.append(
                "SECRET_KEY is randomly generated. Set SECRET_KEY environment variable for production."
            )
