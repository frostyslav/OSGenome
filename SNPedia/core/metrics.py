...
SNP_QUERIES = Counter(
    "snp_queries_total", "Total number of SNP queries processed", ["query_type"]
)
...
# RSID count metrics
RSID_COUNTS = Gauge("rsid_counts", "Number of RSIDs by category", ["category"])
...
def update_rsid_counts(total: int, interesting: int, uncommon: int) -> None:
    ...
    logger.debug(
        f"Updated RSID counts: total={total}, interesting={interesting}, uncommon={uncommon}"
    )
