...
from SNPedia.utils.file_utils import load_from_file, export_to_file
...
class ResultRepository(BaseRepository):
    ...
    def get_statistics(self) -> Dict[str, int]:
        ...
        total = len(data)
        interesting = sum(1 for item in data 
                         if item.get("IsInteresting", "").lower() == "yes")
        uncommon = sum(1 for item in data 
                      if item.get("IsUncommon", "").lower() == "yes")
    ...
    def _dict_to_enriched_snp(self, data: Dict[str, Any]) -> EnrichedSNP:
        ...
        return EnrichedSNP(
            ...
            variations=data.get("Variations", "").split("<br>") if data.get("Variations") else [],
