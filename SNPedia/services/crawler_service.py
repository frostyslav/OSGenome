...
from SNPedia.data.repositories import SNPediaRepository, SNPRepository
...
class CrawlerService:
    ...
    async def crawl_snps_async(self, rsids: Dict[str, str]) -> Dict[str, Dict]:
        ...
        rsids_to_fetch = [
            rsid for rsid in rsids.keys() if rsid not in self.rsid_info.keys()
        ]
        total_rsids = len(rsids)
        new_rsids = len(rsids_to_fetch)

        logger.info(
            f"Starting async crawl: {new_rsids} new RSIDs out of {total_rsids} total"
        )
        logger.info(
            f"Using {max_concurrent} concurrent requests with {self.config.REQUEST_DELAY}s delay"
        )
        ...
        async with aiohttp.ClientSession(
            ...
            for coro in asyncio.as_completed(tasks):
                ...
                if (
                    completed > 0
                    and completed % self.config.SAVE_PROGRESS_INTERVAL == 0
                ):
    ...
    def crawl_snps_sync(self, rsids: Dict[str, str]) -> Dict[str, Dict]:
        ...
        for rsid in rsids.keys():
            ...
            if (
                delay_count > 0
                and delay_count % self.config.SAVE_PROGRESS_INTERVAL == 0
            ):
                logger.info("Exporting intermediate results...")
                self._export_results()
    ...
    async def _fetch_rsid_async(
        self, session: aiohttp.ClientSession, rsid: str
    ) -> Optional[Dict]:
            ...
            try:
                async with session.get(url) as response:
                    ...
                    if response.status == 429:
                        ...
                        logger.warning(
                            f"Rate limited (429) for {rsid}, waiting {backoff}s"
                        )
                    ...
                    if response.status in (502, 503, 504):
                        backoff = self.config.RETRY_DELAY * (2**attempt)
                        logger.warning(
                            f"Server error ({response.status}) for {rsid}, waiting {backoff}s"
                        )
                        if attempt < self.config.MAX_RETRIES - 1:
                            await asyncio.sleep(backoff)
                            continue
                ...
                if attempt < self.config.MAX_RETRIES - 1:
                    ...
                    logger.error(
                        f"Failed to fetch {rsid} after {self.config.MAX_RETRIES} attempts"
                    )
