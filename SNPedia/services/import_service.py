...
class ImportService:
    ...
    def import_genome_file(self, file_path: str) -> Optional[PersonalGenome]:
        ...
        try:
            ...
            if file_size > self.config.MAX_FILE_SIZE_IMPORT:
                raise ValueError(f"File too large: {file_size} bytes (max {self.config.MAX_FILE_SIZE_IMPORT})")
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for line in file:
                    ...
                    if line_count > self.config.MAX_LINE_COUNT:
                        logger.warning(f"File exceeds {self.config.MAX_LINE_COUNT} lines, truncating")
                        break
    ...
    def _get_known_rsids(self) -> list:
        ...
        if os.path.exists("SNPedia/data/snpedia_snps.json"):
            try:
                with open("SNPedia/data/snpedia_snps.json", 'r') as f:
                    return json.load(f)
            except Exception as e:
                ...
                for attempt in range(self.config.MAX_RETRIES):
                    try:
                        ...
                        if attempt < self.config.MAX_RETRIES - 1:
                            ...
                            logger.warning(f"Stopping after {len(known_rsids)} SNPs due to timeouts")
                            cmcontinue = None
                            break
                        ...
                        if attempt < self.config.MAX_RETRIES - 1:
                            ...
                            logger.warning(f"Stopping after {len(known_rsids)} SNPs due to network errors")
                            cmcontinue = None
                            break
