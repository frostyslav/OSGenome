...
def test_config_loading():
    ...
    from SNPedia.core.config import get_config, DevelopmentConfig, ProductionConfig, TestingConfig
...
def test_config_to_dict():
    ...
    # Check required keys
    required_keys = [
        'APP_NAME', 'APP_VERSION', 'MAX_CONTENT_LENGTH',
        'ALLOWED_EXTENSIONS', 'REQUEST_DELAY', 'MAX_RETRIES'
    ]
...
def test_env_variable_parsing():
    ...
    from SNPedia.core.config import get_env_int, get_env_float, str_to_bool
...
def test_flask_integration():
    ...
    try:
        ...
        if app.config.get('MAX_CONTENT_LENGTH'):
            print(f"  ✓ Flask app has MAX_CONTENT_LENGTH ({app.config['MAX_CONTENT_LENGTH']} bytes)")
        else:
            print("  ✗ Flask app missing MAX_CONTENT_LENGTH")
            return False
        
        if app.config.get('ALLOWED_EXTENSIONS'):
            print(f"  ✓ Flask app has ALLOWED_EXTENSIONS ({app.config['ALLOWED_EXTENSIONS']})")
        else:
